...
- 内描边：叠加 1px 半透明白色内边框，纵向渐变（顶部高光 → 底部收暗，模拟环境光自上而下的玻璃边缘反光）：浅色模式顶部 alpha 0.45 → 底部 0.20，深色模式顶部 0.30 → 底部 0.10
...
- 元素水平间距（四列布局）：单词与音标列间距 8pt，音标与释义列间距 16pt，释义与按钮列间距 16pt，释义行间距 6pt
