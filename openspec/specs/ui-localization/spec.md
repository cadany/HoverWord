...
用户修改界面语言后，界面 SHALL 立即切换到新语言，无需重启应用。系统 SHALL 通过语言变更通知刷新所有界面文案：SwiftUI 视图随语言状态重渲染，悬浮窗（AppKit）监听通知手动刷新。语言切换 SHALL NOT 影响设置窗口标题栏样式（重建范围限定在内容子树，NavigationSplitView 本体保持存活，不重装标题栏 chrome）。
...
#### Scenario: 标题栏样式不变
- **WHEN** 用户在任意语言间切换（含"跟随系统"）
- **THEN** 设置窗口标题栏样式 SHALL 与切换前保持一致，不出现独立不透明条带等异常
