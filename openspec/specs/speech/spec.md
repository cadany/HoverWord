...
### Requirement: 发音语音选择
系统 SHALL 在发音设置中按语言分区展示当前系统可用的 TTS 语音下拉列表（过滤系统趣味语音，按音质排序并标记 premium / enhanced），用户可为每种语言独立选择具体的语音。当前仅显示英语分区，未来当用户单词本中出现其他语种时，系统 SHALL 自动新增对应的语言分区。
...
- **THEN** 发音设置 SHALL 在下次打开时自动新增日语分区，展示系统可用的日语语音列表
