...
系统 SHALL 提供背记规则设置 Tab 页，包含背记模式选择、Section 设置（合并 Section 词数、走马灯循环轮次、Section 顺序、Section 内展示顺序）、单单词停留时长、全屏自动隐藏等配置项。
...
- **WHEN** 用户打开背记规则 Tab
- **THEN** 系统 SHALL 显示"Section 设置"卡片，四行均为「左标签 + 右控件」结构："Section 词数"（全局，始终可编辑，1-500，默认 20）、"走马灯循环轮次"（仅走马灯模式下可交互；记忆反馈模式下 SHALL 禁用并降低不透明度至 0.5 以示提示）、"Section 顺序"（分段三选项：顺序开始 / 随机起点 / 随机打乱，默认顺序开始）、"Section 内展示顺序"（分段两选项：顺序播放 / 随机播放）
...
- **WHEN** 用户在背记设置页选择 Section 内展示顺序（顺序/随机）
- **THEN** 仅 SHALL 影响 Section 内部单词顺序（语义与 Section 间策略正交）
