...
系统 SHALL 提供单词本管理 Tab 页，展示单词本列表，支持新建、删除（带确认）、重命名、启用勾选，以及 TXT 词库导入与导出操作。显示每个单词本的单词总数与 Section 数量。
...
- **THEN** 行右侧 SHALL 淡入显示操作区：预览图标按钮（`list.bullet`）+ 竖三点菜单（自绘三圆点；普通单词本菜单含导入、导出、重命名、删除项，删除为红色破坏性样式且与上方以分隔线隔开；空词本的导出项禁用）
- **WHEN** 鼠标离开且该行未选中
- **THEN** 操作区 SHALL 淡出隐藏
...
- **WHEN** 单词本列表包含"我的收藏"
- **THEN** 系统 SHALL 在列表中标记该单词本为系统内置；该行 SHALL 显示预览入口（只读预览）与仅含"导出"的 `...` 菜单（空收藏夹时导出项禁用），SHALL NOT 显示导入、重命名、删除入口
