...
│       ├── SidebarItem.swift              # Sidebar 导航项模型
│       ├── GlassStyles.swift              # 玻璃卡片等共享样式
│       ├── WordbookTabView.swift          # 单词本管理 Tab
│       ├── WordbookPreviewView.swift      # 单词本预览视图
│       ├── ReciteSettingsView.swift       # 背记规则设置 Tab
│       ├── AppearanceView.swift           # 外观设置 Tab
│       ├── SpeechSettingsView.swift       # 发音设置 Tab
│       ├── ExperienceSettingsView.swift   # 体验设置 Tab（转场动效）
│       ├── GeneralSettingsView.swift      # 通用设置 Tab（界面语言）
│       └── ScrollBackgroundCompat.swift   # 滚动背景兼容性适配
├── Models/                       # 数据模型层
│   ├── Entities/                 # Core Data 实体类
│   │   ├── Wordbook+CoreDataClass.swift
│   │   ├── Wordbook+CoreDataProperties.swift
│   │   ├── WordEntry+CoreDataClass.swift
│   │   ├── WordEntry+CoreDataProperties.swift
│   │   ├── Favorite+CoreDataClass.swift
│   │   └── Favorite+CoreDataProperties.swift
│   ├── Enums/
│   │   ├── ReciteMode.swift      # 背记模式枚举
│   │   ├── PlayOrder.swift       # 组内播放顺序枚举
│   │   ├── SectionOrder.swift    # 组间顺序枚举
│   │   └── ContentVisibility.swift # 内容显示模式枚举
...
│   ├── WordbookExportService.swift # 单词本导出服务
│   ├── LanguageDetectionService.swift # 语种自动检测服务
...
- 全应用以 Liquid Glass 玻璃材质为基础：macOS 26+ 使用 `.liquid` / `.glassEffect`，14-25 使用系统默认材质，自动跟随深浅色模式

- 悬浮窗为无边框圆角玻璃窗（8pt 连续圆角），设置窗为标准标题栏 + 整窗透明（`isOpaque=false, backgroundColor=.clear`）让 SwiftUI material 透出
...
- 数据模型不硬编码语言，通过 source\_lang / target\_lang 字段标识
